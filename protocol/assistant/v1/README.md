...
sorts Powers by id, fixes every route to `POST /v1/powers/<id>`, and records the exact human-request
capabilities declared by that Power. An undeclared capability never acquires a prompt channel.
...
`invocation.schema.json` contains the validated Power input, invocation-scoped Integration bearer tokens,
...
`{"type":"result","result":{...}}`; the SDK validates `result` against the reviewed Power output schema.
A capability-declared request is `{"type":"request","request":{...}}`, with one closed request kind,
ordinal, canonical fingerprint, and bounded inert copy. Team accepts it only for the exact reviewed Power,
