...
class ChatProgressTests(unittest.TestCase):
    ...
    def test_orchestrator_reports_real_model_action_and_validation_operations(self) -> None:
        events: list[dict[str, object]] = []
        ...
        self.assertEqual(
            ...
            [
                ...
                ("action-preparation", "finished"),
                ("action", "started"),
                ("action", "finished"),
                ("model", "started"),
                ("model", "finished"),
                ("action-delivery", "started"),
                ("action-delivery", "finished"),
                ("team-context", "started"),
                ("team-context", "finished"),
            ],
        )
        action_events = [event for event in events if event["phase"] == "action"]
        self.assertTrue(all(event["index"] == event["total"] == 1 for event in action_events))
        self.assertTrue(
            all((event["assistant_id"], event["action"]) == ("hello-pulse", "hello") for event in action_events)
        )

    def test_reporter_rejects_identity_outside_action_and_invalid_identifiers(self) -> None:
        reporter = chat_progress.Reporter([].append)

        with (
            self.assertRaisesRegex(ValueError, "progress identity"),
            reporter.span("model", assistant_id="helper", action="lookup"),
        ):
            pass
        with (
            self.assertRaisesRegex(ValueError, "progress identity"),
    ...
    def test_resumed_batch_repeats_preparation_then_reports_action_identity(self) -> None:
        ...
        self.assertFalse(any(event["phase"] == "action" for event in events))
        ...
        self.assertEqual(phases.count("action-preparation"), 2)
        action_events = [event for event in events if event["phase"] == "action"]
